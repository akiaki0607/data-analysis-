...
class SmartXiaohongshuScraper:
    ...
    async def try_access_xiaohongshu(self):
        """尝试多种方式访问小红书"""
        print("\n" + "="*60)
        print("🚀 尝试访问小红书网站...")
        print("="*60)
        ...
        # 方法1: 尝试多个URL
        for i, url in enumerate(self.xiaohongshu_urls, 1):
            try:
                print(f"\n🔄 尝试方法 {i}: {url}")
                ...
                # 检查是否有搜索框
                try:
                    search_input = await self.page.query_selector('input[placeholder*="搜索"], input[class*="search"], input[data-testid*="search"]')
                    if search_input:
                        print("   ✅ 发现搜索框，页面正常")
                        return True
                    else:
        ...
        # 方法2: 手动访问
        print(f"\n🔧 自动访问失败，请手动操作：")
        print("1. 在打开的浏览器中手动访问 https://www.xiaohongshu.com/explore")
    ...
    async def extract_suggestions_from_page(self, keyword: str) -> List[str]:
        ...
        try:
                ...
                try:
                    ...
                    for element in elements:
                        try:
                            ...
                            if text and text.strip() and text.strip() != keyword and keyword in text:
            ...
            # 如果没有找到联想词，尝试查找包含关键词的文本
            if not suggestions:
                try:
                    # 查找包含关键词的所有文本元素
                    all_text_elements = await self.page.query_selector_all('span, div, p, a')
                    
                    for element in all_text_elements[:50]:  # 限制检查数量
                        try:
                            text = await element.inner_text()
                            if (text and text.strip() and 
                                keyword in text and 
                                text.strip() != keyword and
                                len(text.strip()) < 50):  # 排除过长的文本
                                suggestions.append(text.strip())
                                if len(suggestions) >= 5:  # 最多取5个
                                    break
                        except:
                            continue
                            
                except Exception as e:
                    print(f"   ⚠️  文本元素搜索失败: {str(e)}")
    ...
    def generate_fallback_suggestions(self, keyword: str) -> List[str]:
        ...
        return fallback_templates[:5]  # 返回5个兜底建议
    ...
    async def process_keywords(self, keywords_data: List[Dict], output_dir: str):
        ...
        for i, row in enumerate(keywords_data, 1):
            ...
            current_url = self.page.url if self.page else f"https://www.xiaohongshu.com/search_result?keyword={keyword}"
    ...
    def save_results(self, results: Dict, output_dir: str):
        ...
        for client, data in results.items():
            if data:
                filename = f"{client}_xiaohongshu.csv"
    ...
    async def run(self, input_file: str, output_dir: str, screenshot_dir: str):
        ...
        try:
            ...
            # 尝试访问小红书
            if not await self.try_access_xiaohongshu():
                print("❌ 无法访问小红书网站")
                return
...
async def main():
    parser = argparse.ArgumentParser(description='智能小红书联想词采集工具')
    ...
    # 运行采集器
    scraper = SmartXiaohongshuScraper(headful=args.headful)
