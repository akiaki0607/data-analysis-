...
class DouyinScraper:
    ...
    async def init_browser(self):
        ...
        self.browser = await self.playwright.chromium.launch(headless=not self.headful)
    ...
    async def navigate_to_douyin(self):
        """导航到抖音首页"""
        try:
            douyin_config = self.config['douyin']
            await self.page.goto(douyin_config['base_url'], timeout=douyin_config['timeouts']['page_load'])
            await self.page.wait_for_load_state('networkidle')
            self.logger.info("成功导航到抖音首页")
            return True
        except Exception as e:
    ...
    async def search_keyword(self, keyword):
        ...
        try:
            ...
            # 等待搜索框出现
            await self.page.wait_for_selector(selectors['search_input'], timeout=timeouts['search_input'])
            
            # 清空搜索框并输入关键词
            search_input = self.page.locator(selectors['search_input'])
            await search_input.clear()
            await search_input.fill(keyword)
            
            # 等待联想词出现
            await asyncio.sleep(1)  # 给联想词一些时间加载
            
            # 尝试等待联想词容器出现
            try:
                await self.page.wait_for_selector(selectors['suggest_container'], timeout=timeouts['suggestions'])
            except:
                self.logger.warning(f"联想词容器未找到，关键词: {keyword}")
                return [], None
            
            # 截图
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            screenshot_filename = create_screenshot_filename(keyword, timestamp)
        ...
        try:
            ...
            for i, element in enumerate(suggest_elements[:10]):  # 最多取10个
                try:
                    ...
                    self.logger.warning(f"提取联想词失败 (DOM方法): {str(e)}")
            ...
            if self.ocr.is_available():
                ...
                self.logger.warning("OCR不可用，无法提取联想词")
                return []
    ...
    async def _extract_suggestions_ocr(self, keyword):
        ...
        try:
            # 截图联想词区域
            suggest_container = await self.page.query_selector(self.config['douyin']['selectors']['suggest_container'])
            if not suggest_container:
                return []
            
            # 临时截图文件
            temp_screenshot = f"temp_ocr_{keyword}_{int(time.time())}.png"
            await suggest_container.screenshot(path=temp_screenshot)
    ...
    async def process_keywords(self, keywords_df, output_dir, screenshot_dir):
        ...
        for client in keywords_df['client'].unique():
            ...
            for _, row in client_keywords.iterrows():
                ...
                # 重试机制
                max_attempts = self.config['douyin']['retry']['max_attempts']
                    ...
                    try:
                        ...
                        if not suggestions:
                            if attempt == max_attempts - 1:
                                ...
                                await asyncio.sleep(self.config['douyin']['retry']['delay'] / 1000)
                                continue
...
def main():
    parser = argparse.ArgumentParser(description='抖音联想词自动化采集工具')
    ...
    async def run_scraper():
        scraper = DouyinScraper(headful=args.headful)
